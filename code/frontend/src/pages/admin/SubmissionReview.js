...
const SubmissionReview = () => {
  ...
  const handleReviewClient = async (client) => {
    ...
    try {
      ...
      const submissionsResponse = await submissionApi.getSubmissionsByClient(client.id);
      ...
      if (clientSubs.length > 0) {
        ...
        for (const question of offeringQuestions) {
          try {
            const weightResponse = await questionWeightsApi.getQuestionWeights(question.id);
    ...
    <div className="submission-review-page">
      <div className="page-header">
        <div>
          ...
          <p className="subtitle">Review and classify customer applications</p>
        </div>
      </div>

      <Card title="Pending Applications">
        {clients.length === 0 ? (
          <p className="no-data">No applications to review.</p>
        ) : (
            ...
            <div className="review-section">
              ...
              <div className="info-grid">
                <div className="info-item">
                  <strong>Name:</strong> {selectedClientData?.firstName} {selectedClientData?.lastName}
                </div>
                <div className="info-item">
                ...
                <div className="info-item">
                  <strong>Birth Date:</strong> {selectedClientData?.birthDate ? new Date(selectedClientData.birthDate).toLocaleDateString() : 'N/A'} ({selectedClientData?.birthDate ? calculateAge(selectedClientData.birthDate) : 'N/A'} years old)
                </div>
                <div className="info-item">
                  <strong>Gender:</strong> {selectedClientData?.gender}
                </div>
                <div className="info-item">
                  <strong>Address:</strong> {selectedClientData?.address}, {selectedClientData?.city} {selectedClientData?.zip}, {selectedClientData?.canton}
                </div>
              </div>
            </div>
              ...
              <div className="answers-list">
                {clientSubmissions.map((submission) => (
                  <div key={submission.id} className="answer-item">
                    <div className="question-title">{submission.question.title}</div>
                    ...
                    {weights[submission.question.id]?.find(
                      ...
                      <div className="answer-weight">
                        Weight: {weights[submission.question.id].find(
                          (w) => w.valueQuestion === submission.value
                        ).scoreQuestion}
                      </div>
                    )}
            ...
            <div className="review-section">
              ...
              <div className="risk-assessment">
                <div className="risk-score">
                  ...
                  <span className={`score ${riskScore < 1.0 ? 'low' : riskScore < 2.0 ? 'medium' : 'high'}`}>
                    {riskScore.toFixed(2)}
                  </span>
                </div>
                  ...
                  <div className="weight-breakdown">
                    ...
                    {calculatedWeights.map((item, index) => (
                      <div key={index} className="weight-item">
                        ...
                        <span className="weight-value">{item.weight > 0 ? '+' : ''}{item.weight}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="review-section">
                ...
                <div className="ai-analysis">
                  <div className="ai-recommendation">
                    <strong>Recommendation:</strong> {aiClassification.recommendation}
                  </div>
                  <div className="ai-confidence">
                    <strong>Confidence:</strong> {(aiClassification.confidence * 100).toFixed(1)}%
                  </div>
                  {aiClassification.analysis && (
                    <div className="ai-findings">
                      <strong>Key Findings:</strong> {aiClassification.analysis.key_findings}
                    </div>
                  )}
                </div>
              </div>
